...
struct HuffmanCodec<E, TIMING>::impl {
  ...
  impl(
      size_t const inlen, int const _bklen, int const _pardeg, bool use_HFR,
      bool debug)
  {
    ...
    buf = new internal_buffer(inlen, _bklen, _pardeg, use_HFR, debug);
    hist = new memobj<u4>(
...
#define HFR_ENCODE(REDUCE)                                               \
  ...
  phf::cu_hip::GPU_HFReVISIT_encode<E, HFR_Magnitude, REDUCE, false, H>( \
...
PHF_TPL PHF_CLASS::HuffmanCodec(
    size_t const inlen, int const bklen, int const pardeg, bool use_HFR,
    bool debug) :
